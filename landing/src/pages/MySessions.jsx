import { useEffect, useMemo, useState } from "react";
import axios from "axios";
import { Check, X, Star, GraduationCap, BookOpen } from "lucide-react";
import { AppShell } from "@/components/AppShell";
import { CalendarMenu } from "@/components/CalendarMenu";
import { JoinMeetingButton } from "@/components/JoinMeetingButton";
import { Button } from "@/components/ui/Button";
import { Badge } from "@/components/ui/Badge";
import { Select } from "@/components/ui/Select";
import { Textarea } from "@/components/ui/Textarea";
import { Label } from "@/components/ui/Label";
import { Alert } from "@/components/ui/Alert";
import { useAuth } from "@/context/AuthContext";
import { api } from "@/lib/api";

export default function MySessions() {
  const { user, refreshUser } = useAuth();
  const [sessions, setSessions] = useState([]);
  const [review, setReview] = useState({
    session_id: null,
    rating: 5,
    comment: "",
  });
  const [error, setError] = useState("");
  const [reviewError, setReviewError] = useState("");

  const closeReview = () => {
    setReview({ session_id: null, rating: 5, comment: "" });
    setReviewError("");
  };

  const fetchSessions = async () => {
    try {
      const res = await api.get("/sessions/my");
      setSessions(res.data);
    } catch {
      // silent
    }
  };

  useEffect(() => {
    fetchSessions();
  }, []);

  const updateStatus = async (id, status) => {
    setError("");
    try {
      await api.put(`/sessions/${id}/status`, { status });
      fetchSessions();
      refreshUser();
    } catch (err) {
      const msg = axios.isAxiosError(err)
        ? err.response?.data?.message ?? "Failed to update"
        : "Failed to update";
      setError(msg);
    }
  };

  const submitReview = async () => {
    if (review.rating < 1 || review.rating > 5) {
      setReviewError("Please choose a rating between 1 and 5 stars.");
      return;
    }
    setReviewError("");
    try {
      await api.post("/reviews", review);
      closeReview();
      fetchSessions();
    } catch (err) {
      const msg = axios.isAxiosError(err)
        ? err.response?.data?.message ?? "Failed to submit review"
        : "Failed to submit review";
      setReviewError(msg);
    }
  };

  const { teaching, learning } = useMemo(() => {
    const t = [];
    const l = [];
    for (const s of sessions) {
      if (user && s.mentor_id === user.id) t.push(s);
      else if (user && s.learner_id === user.id) l.push(s);
    }
    return { teaching: t, learning: l };
  }, [sessions, user]);

  return (
    <AppShell
      title={
        <>
          My <span className="font-serif italic font-normal">sessions</span>
        </>
      }
      subtitle="Track what you're teaching and what you're learning."
    >
      {error && (
        <Alert variant="error" className="mb-6">
          {error}
        </Alert>
      )}

      <SessionSection
        icon={<GraduationCap className="w-4 h-4" />}
        title="Teaching"
        description="Sessions where you're the mentor."
        emptyMessage="You're not teaching any sessions yet. When a learner books a slot you've opened, it'll appear here."
        rows={teaching}
        otherPartyLabel="Learner"
        getOtherPartyName={(s) => s.learner_name}
        onComplete={(id) => updateStatus(id, "completed")}
        onCancel={(id) => updateStatus(id, "cancelled")}
        onReview={undefined}
      />

      <SessionSection
        icon={<BookOpen className="w-4 h-4" />}
        title="Learning"
        description="Sessions you've booked with other mentors."
        emptyMessage="You haven't booked any sessions yet. Head to the Marketplace to find a mentor."
        rows={learning}
        otherPartyLabel="Mentor"
        getOtherPartyName={(s) => s.mentor_name}
        onComplete={(id) => updateStatus(id, "completed")}
        onCancel={(id) => updateStatus(id, "cancelled")}
        onReview={(id) =>
          setReview({ session_id: id, rating: 5, comment: "" })
        }
      />

      {review.session_id !== null && (
        <div
          className="fixed inset-0 z-[60] bg-background/85 backdrop-blur-sm flex items-center justify-center px-6"
          onClick={() => closeReview()}
        >
          <div
            className="liquid-glass rounded-3xl p-8 md:p-10 w-full max-w-md"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-2xl font-medium tracking-[-0.5px] mb-6">
              Leave a <span className="font-serif italic font-normal">review</span>
            </h3>
            <div className="space-y-5">
              {reviewError && <Alert variant="error">{reviewError}</Alert>}
              <div>
                <Label htmlFor="rating">Rating</Label>
                <Select
                  id="rating"
                  value={review.rating}
                  onChange={(e) =>
                    setReview({ ...review, rating: Number(e.target.value) })
                  }
                >
                  {[5, 4, 3, 2, 1].map((n) => (
                    <option key={n} value={n}>
                      {"★".repeat(n)}
                      {"☆".repeat(5 - n)} ({n})
                    </option>
                  ))}
                </Select>
              </div>
              <div>
                <Label htmlFor="comment">Comment</Label>
                <Textarea
                  id="comment"
                  placeholder="Share your experience…"
                  value={review.comment}
                  onChange={(e) =>
                    setReview({ ...review, comment: e.target.value })
                  }
                />
              </div>
              <div className="flex gap-3 pt-2">
                <Button onClick={submitReview} className="flex-1">
                  Submit Review
                </Button>
                <Button variant="outline" onClick={closeReview}>
                  Cancel
                </Button>
              </div>
            </div>
          </div>
        </div>
      )}
    </AppShell>
  );
}

function SessionSection({
  icon,
  title,
  description,
  emptyMessage,
  rows,
  otherPartyLabel,
  getOtherPartyName,
  currentUserIsLearner,
  onComplete,
  onCancel,
  onReview,
}) {
  return (
    <section className="mb-12 last:mb-0">
      <div className="flex items-start gap-3 mb-5">
        <div className="w-9 h-9 rounded-full liquid-glass flex items-center justify-center text-foreground shrink-0">
          {icon}
        </div>
        <div>
          <h2 className="text-xl font-medium tracking-[-0.5px]">{title}</h2>
          <p className="text-sm text-muted-foreground mt-0.5">{description}</p>
        </div>
        <span className="ml-auto text-xs uppercase tracking-[2px] text-muted-foreground mt-2">
          {rows.length} {rows.length === 1 ? "session" : "sessions"}
        </span>
      </div>

      {rows.length === 0 ? (
        <div className="border border-dashed border-border/50 rounded-2xl px-6 py-10 text-center text-muted-foreground text-sm">
          {emptyMessage}
        </div>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-border/50">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-secondary/40 text-xs uppercase tracking-[2px] text-muted-foreground">
                <Th>Skill</Th>
                <Th>{otherPartyLabel}</Th>
                <Th>Time</Th>
                <Th>Status</Th>
                <Th className="text-right">Actions</Th>
              </tr>
            </thead>
            <tbody>
              {rows.map((s, i) => (
                <tr
                  key={s.id}
                  className={i < rows.length - 1 ? "border-b border-border/40" : ""}
                >
                  <Td className="font-medium">{s.skill_name}</Td>
                  <Td>{getOtherPartyName(s)}</Td>
                  <Td>{new Date(s.start_time).toLocaleString()}</Td>
                  <Td>
                    <Badge
                      tone={
                        s.status === "completed"
                          ? "default"
                          : s.status === "cancelled"
                            ? "danger"
                            : "outline"
                      }
                    >
                      {s.status}
                    </Badge>
                  </Td>
                  <Td className="text-right">
                    <div className="inline-flex gap-2 justify-end flex-wrap">
                      {s.status === "booked" && (
                        <>
                          <JoinMeetingButton session={s} />
                          <CalendarMenu
                            session={s}
                            iAmMentor={!currentUserIsLearner}
                          />
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => onComplete(s.id)}
                          >
                            <Check className="w-3.5 h-3.5" />
                            Complete
                          </Button>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => onCancel(s.id)}
                          >
                            <X className="w-3.5 h-3.5" />
                            Cancel
                          </Button>
                        </>
                      )}
                      {s.status === "completed" && onReview && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => onReview(s.id)}
                        >
                          <Star className="w-3.5 h-3.5" />
                          Review
                        </Button>
                      )}
                    </div>
                  </Td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </section>
  );
}

function Th({ children, className = "" }) {
  return (
    <th
      className={`text-left font-medium px-5 py-3.5 first:pl-6 last:pr-6 ${className}`}
    >
      {children}
    </th>
  );
}
function Td({ children, className = "" }) {
  return (
    <td
      className={`px-5 py-4 text-foreground/90 first:pl-6 last:pr-6 ${className}`}
    >
      {children}
    </td>
  );
}
