...
export default function Profile() {
  ...
  useEffect(() => {
    if (!id) return;
    api.get<UserProfile>(`/users/profile/${id}`).then((r) => setProfile(r.data)).catch(() => {});
    api.get<Review[]>(`/reviews/mentor/${id}`).then((r) => setReviews(r.data)).catch(() => {});
    if (isOwnProfile) {
  ...
  const reload = () => {
    if (!id) return;
    api.get<UserProfile>(`/users/profile/${id}`).then((r) => setProfile(r.data));
  };
        ...
        <Section title="Add a Skill">
          ...
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <Select
              value={skillType}
              onChange={(e) =>
                setSkillType(e.target.value as "offer" | "learn")
              }
